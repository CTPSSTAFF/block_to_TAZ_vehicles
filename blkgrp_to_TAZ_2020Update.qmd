...
# Test a small scale example.
# https://stackoverflow.com/questions/52522872/r-sf-package-centroid-within-polygon/55099535#55099535
...
rooftops_filt <- rooftops |> tail(1000)

rooftops_filt_pos <- rooftops_filt |>
  st_point_on_surface()

rooftops_filt_cent <- rooftops_filt |>
  st_centroid()

mapview(rooftops_filt_pos, col.regions = "green") + 
     mapview(rooftops_filt_cent, col.regions = "red") + 
     mapview(rooftops_filt, col.regions = "grey50")

# rooftops_cent <- rooftops |> st_centroid()
# saveRDS(rooftops_cent, "./data/processed/rooftops_cent.rds")
rooftops_cent <- readRDS("./data/processed/rooftops_cent.rds") |> select(STRUCT_ID)

# rooftops_pos <- rooftops |> st_point_on_surface()
# saveRDS(rooftops_pos, "./data/processed/rooftops_pos.rds")
rooftops_pos <- readRDS("./data/processed/rooftops_pos.rds") |> select(STRUCT_ID)

mapview(rooftops |> filter(STRUCT_ID == "53226_895084")) +
  mapview(rooftops_cent |> filter(STRUCT_ID == "53226_895084"), col.regions = "red") + 
    mapview(rooftops_pos |> filter(STRUCT_ID == "53226_895084"), col.regions = "green")

rooftops <- rooftops_pos

rm(rooftops_cent, rooftops_pos)

gc()
```

We can make our work faster by pre-filtering the list of roofs in the Boston MPO. By using the buffers we should be able to include all the roofs of concern.

```{r}
TAZ_MPO_buffer_summ <- TAZ_MPO_buffer |>
  ungroup() |> 
  summarize()

blk_group_MPO_buffer_summ <- blk_group_MPO_buffer |>
  ungroup() |>  
  summarize()

full_buffer <- st_union(blk_group_MPO_buffer_summ,
                        TAZ_MPO_buffer_summ) |>
  summarize()
...
# tictoc::tic()
# roof_filt <- st_is_within_distance(rooftops, full_buffer, dist = 0, sparse = FALSE)
# tictoc::toc()
# rooftops_MPO <- rooftops[roof_filt == TRUE,]
...
repeat_roof <- repeat_roof |>
  mutate(STRUCT_ID = coalesce(
    STRUCT_ID, 
    paste0(row_number(), "_replaced_na")
    ))
...
write_csv(roof_shares_ratio, 
          file = "./output/data/rooftop_allocation_MPO_largest_in_each.csv")
...
percentages_ID <- TAZ_results %>% 
  ...
  select(ID, LA_pct_min, FA_pct_min, RT_pct_min )
...
mpct_FA <- mapview(TAZ_map, zcol = "FA_pct_min", at = seq(0,100,20),lwd = 0, legend = FALSE)
mpct_RT <- mapview(TAZ_map, zcol = "RT_pct_min", at = seq(0,100,20),lwd = 0, legend = FALSE)
mpct_BG <- mapview(block_group_map, zcol = "pct_min", at = seq(0,100,20),lwd = 0, legend = FALSE)

sync(mpct_LA, mpct_FA, mpct_RT, mpct_BG) # 4 panels synchronised
