...
#   mapview(TAZ_MPO |> st_union(), alpha.regions = 0.01, lwd = 3) +
#   mapview(TAZ_MPO_buffer) + 
#   mapview(TAZ_buffer, alpha.regions = 0.01, lwd = 3)
```
...
water2 <- water |> arrange(desc(AWATER)) |> 
  mutate(share_of_area = cumsum(AWATER)/sum(AWATER),
         ntile = row_number()/n())

ggplot(water2, aes(x= ntile, y = share_of_area)) + 
  geom_point() +
