...
roof_taz <- saveRDS("./data/processed/roof_taz.rds")

tictoc::tic()
roof_shares <- rooftops |> 
  filter(STRUCT_ID %in% roof_cbg$STRUCT_ID) |> 
...
mapview(roof_shares |> filter(ID %in% c(2708, 2709))) + 
  mapview(blk_groups_land, col.regions = "pink")

```

```{r}
# TODO: Check why some roofs are in cbg but not taz and viceversa 
# TODO: Check
```
