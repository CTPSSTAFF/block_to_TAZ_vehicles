...
blk_group_ply$TOTAL_BG_AREA <- st_area(blk_group_ply)
...
intersections_land <- intersections_land %>% 
  ...
  mutate(LAND_BG_AREA = sum(INTERSECTION_LAND_AREA))

library(ggplot2)
ggplot(intersections_land, aes(x = LAND_BG_AREA, y = ALAND)) + geom_smooth(method = "lm") + geom_point() + coord_equal() +  geom_abline(slope = 1) 
```
...
intersections_land <- intersections_land %>% 
  select(Intersection_ID, ID, GEOID, TOTAL_BG_AREA, INTERSECTION_LAND_AREA, LAND_BG_AREA)
...
num_rooftops2 <- roofs_in_intersect %>% 
  st_drop_geometry() %>% 
  group_by(Intersection_ID) %>% 
  mutate(num_roof = n()) %>% 
  distinct(Intersection_ID, ID, GEOID, INTERSECTION_LAND_AREA, num_roof)
