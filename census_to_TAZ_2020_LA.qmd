...
minority_acs_tidy <- get_acs(geography = "tract",
                       ...
                       state= "MA",
                       year= 2021)  %>% 
  rename(nonmin_pop = estimate, tot_pop = summary_est, nonmin_moe = moe, tot_moe = summary_moe ) %>% 
  select(-variable) %>% 
  mutate(GEOID = as.numeric(GEOID))
  
  
  # mutate(nonmin_pct = nonmin_pop / tot_pop, min_pct = 1 - nonmin_pct) %>% 
  # mutate(min_moe_prop = moe_prop(nonmin_pop, tot_pop, nonmin_moe, tot_moe)) 
  #   
...
TAZ_results <- allocation_results |> 
  ...
  # multiply area allocation percentages by estiamtes and moes
  ...
  mutate(tot_moe_alloc = TRACT_LA_pct * tot_moe)


  # sum up estimages and moes for each TAZ
TAZ_results_summ <- TAZ_results %>% 
  ...
  mutate(nonmin_pct = TAZ_nonmin / TAZ_tot, min_pct = 1 - nonmin_pct) %>%
  select(-nonmin_pct) %>% 
  mutate(minpct_moe = moe_prop(TAZ_nonmin, TAZ_tot, TAZ_nonmin_moe, TAZ_tot_moe))  %>% 
  # min value method
  mutate(nonmin_maxval_pct = TAZ_nonmin_maxval / TAZ_tot, min_minval_pct = 1 - nonmin_maxval_pct ) %>% 
